...
async function run() {
    ...
    try {
        ...
        const tableNames = tables.map(t => Object.values(t)[0]).filter(t => t !== 'anuncios');
        ...
        for (const tableName of tableNames) {
            ...
            let createSql = createRows[0]['Create Table'].replace(/utf8mb4_0900_ai_ci/g, 'utf8mb4_unicode_ci');
            await newConn.execute(`DROP TABLE IF EXISTS \`${tableName}\``);
            try { await newConn.execute(createSql); } catch (e) { await newConn.execute(createSql.replace(/COLLATE=\w+/g, '')); }
            ...
            if (data.length > 0) {
                ...
                for (const row of data) {
                    const values = columns.map(c => row[c] instanceof Date ? formatDate(row[c]) : row[c]);
        ...
        console.log('SUCCESS (except anuncios)');
    } catch (e) {
        console.error(e);
    } finally {
